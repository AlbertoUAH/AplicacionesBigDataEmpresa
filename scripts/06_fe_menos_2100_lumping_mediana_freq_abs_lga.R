...
#         1. Aplicar el mismo proceso de la iteracion 5 (reemplazar las categorias por frecuencias absolutas) a lga
# Comentarios: 
#-------------------
...
#- lga
datcompleto[, fe_lga_freq := as.numeric(.N), by = lga]
length(unique(datcompleto[, fe_lga_freq]))  # 120 vs 125 fe_ward


# Eliminamos las columnas originales
datcompleto[, `:=`(fe_funder = NULL, fe_ward = NULL, lga = NULL)]
...
# guardo submission
fwrite(my_sub_10, file = "./submissions/10_05_lumping_sobre_funder_ward_freq_abs_mas_lga.csv")
# 0.8199

knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8159596), 
                        "Data Submission" = c(0.8180, 0.8197, 0.8213, 0.8203, 0.8198, 0.8216, 0.8199),
                        row.names = c("Mejor accuracy en el concurso",
                                      ...
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping + freq. abs. sobre funder + ward (mediana) + lga")),
             align = 'c')

#-- Conclusion: aplicando frecuencias absolutas sobre lga no parece mejorar el modelo
