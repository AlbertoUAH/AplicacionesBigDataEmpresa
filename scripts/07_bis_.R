...
#         1. Aplicar imputacion por Leave One Out Encoding a las variables categoricas
# Comentarios: 
#-------------------
...
suppressPackageStartupMessages({
  ...
  library(cattonum)             # Categorical Encoding Techniques
  library(doParallel)
...
cat_cols <- names(as.data.frame(datcompleto %>% select(where(is.character))))

cl <- makeCluster(detectCores())
registerDoParallel(cl)

# Aplicamos LOO Encoding
train[, paste0(cat_cols, "_loo_end") := lapply(.SD, function(x) encode_leave_one_out(x, train[, status_group])), .SDcols = cat_cols]
...
# guardo submission
fwrite(my_sub_10, file = "./submissions/10_05_lumping_sobre_funder_ward_freq_abs_categoricas.csv")
# 0.8226

knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8157071), 
                        "Data Submission" = c(0.8180, 0.8197, 0.8213, 0.8203, 0.8198, 0.8216, 0.8226),
...
impor_df <- as.data.frame(my_model_10$variable.importance)
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/10_05_lumping_sobre_funder_ward_freq_abs_categoricas.png')
