...
# 0.808771 si no eliminamos las variables
...
# guardo submission
fwrite(my_sub_11, file = "./submissions/11_05_lumping_sobre_funder_ward_target_encoding.csv")
# 0.8118 si no eliminamos las variables
...
# 0.7995118
my_model_11_2 <- fit_random_forest(formula,
                                 train_2)

my_sub_11_2 <- make_predictions(my_model_11_2, test)

# guardo submission
fwrite(my_sub_11_2, file = "./submissions/11_05_lumping_sobre_funder_ward_target_encoding_v2.csv")
# 0.8056

knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8157071, 0.808771, 0.7995118), 
                        "Data Submission" = c(0.8180, 0.8197, 0.8213, 0.8203, 0.8198, 0.8216, 0.8226, 0.8118, 0.8056),
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/11_05_lumping_sobre_funder_ward_freq_abs_categoricas_num_missings_imp.png')
