...
#            y agrupar variables (ejemplo: waterpoint_type y waterpoint_group)
# Comentarios: 
#-------------------
...
#-- Creamos la variable is_lake
datcompleto$fe_basin <- ifelse(
  !is.na(stri_match_all_regex(datcompleto$basin, "Lake")),
  "Lake",
  ifelse(
    !is.na(stri_match_all_regex(datcompleto$basin, "Coast")),
    "Coast",
    ifelse(
      datcompleto[, basin == "Internal"],
      "Internal",
      "Others"
    )
  )
)
datcompleto[, basin := NULL]
...
# guardo submission
fwrite(my_sub_12, file = "./submissions/12_05_lumping_sobre_funder_ward_fe_basin.csv")
# 0.8212

knitr::kable(data.frame("Train accuracy" = c(0.8168687, 0.8101178, 0.8122391, 0.8124579, 0.8122727, 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8158418, 0.8156397, 0.8161111, 0.815404), 
                        "Data Submission" = c(0.8128, 0.8096, 0.8174, 0.8176, 0.8168, 0.8197, 0.8213, 0.8203, 0.8198, 0.8195, 0.8199, 0.8204, 0.8212),
                        row.names = c("Num + Cat (> 1 & < 1000) sin duplicados",
                                      ...
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana) + fe agrupando columnas",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana) + fe is_lake",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana) + fe basin")),
             align = 'c')
