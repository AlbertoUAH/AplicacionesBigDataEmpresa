...
cols <- c('funder', 'ward', 'scheme_name', 'wpt_name', 'subvillage')
...
cat_cols <- names(datcompleto[, which(sapply(datcompleto, is.character)), with = FALSE])[c(-22, -23)]
...
# guardo submission
fwrite(my_sub_16, file = "./submissions/16_13_lumping_sobre_funder_ward_installer_scheme_name_freq_abs_categoricas_mas_wpt_name_subvillage.csv")
...
knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8157071, 0.808771, 0.7995118,
                        ...
                        row.names = c("Mejor accuracy en el concurso",
                                      ...
                                      "Num + Cat (> 1 & < 40000) fe anteriores + subvillage + wpt_name + lumping sobre funder, ward, scheme_name (mediana)")),
             align = 'c')
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/16_13_lumping_sobre_funder_ward_installer_scheme_name_freq_abs_categoricas_mas_wpt_name_subvillage.png')
