...
# Prediccion
my_pred <- predict(my_model, as.data.frame(test), type = "response")

# Submission
my_sub <- data.table(
  id = test$id,
  status_group = my_pred$data$response
)

# 0.8206
fwrite(my_sub, "./submissions/tunning_models/xgboost/autoxgboost.csv")
