...
#         1. Reemplazar las variables categoricas por el promedio de aparicion con respecto a la variable objetivo 
# Comentarios: 
#-------------------
...
# 0.8086364 si no eliminamos las variables
...
# guardo submission
fwrite(my_sub_12, file = "./submissions/12_lumping_fe_sobre_funder_ward_target_encoding_categoricas_sin_eliminar_variables.csv")
# 0.8110 si no eliminamos las variables

#-- Si eliminamos las variables originales
train_2 <- train[, names(datcat_df) := NULL]
test_2 <- test[, names(datcat_df) := NULL]

# 0.8005892
my_model_12_2 <- fit_random_forest(formula,
                                   train_2)

my_sub_12_2 <- make_predictions(my_model_12_2, test)

# guardo submission
fwrite(my_sub_12_2, file = "./submissions/12_lumping_fe_sobre_funder_ward_target_encoding_categoricas_variables_eliminadas.csv")
# 0.8044
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/12_lumping_fe_sobre_funder_ward_target_encoding_categoricas_sin_eliminar_variables.png')

# Analizando el grafico de importancia, llama la atencion que funder, ward y lga hayan aumentando considerablemente su importancia
# con respecto a la imputacion por frecuencias absolutas ¿Y si aplicamos target encoding solo a funder, ward y lga?
