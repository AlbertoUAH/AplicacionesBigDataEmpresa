...
#   ¿Y si aplicamos la media sobre el conjunto train+test?
for (column in c("construction_year", "gps_height", "longitude")) {
  ...
  datcompleto[is.na(get(new_feature)), (new_feature) := mean(datcompleto[, get(new_feature)], na.rm = TRUE)]
...
# guardo submission
fwrite(my_sub_14, file = "./submissions/14_05_lumping_sobre_funder_ward_freq_abs_categoricas_num_imp_media.csv")
# 0.8199
