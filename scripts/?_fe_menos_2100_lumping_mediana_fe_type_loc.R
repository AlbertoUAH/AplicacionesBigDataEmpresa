...
#            e incluir variables categoricas que indique si el poblado es rural, urbano u otro.
# Comentarios: 
#-------------------
...
#-- Nuevas variables
unique(datcompleto$lga) # ¿Es una poblacion rural o urbana?

#- fe_urban
datcompleto$fe_type_location <- ifelse(
  !is.na(stri_match_all_regex(datcompleto$lga, "Urban")),
  "Urban",
  ifelse(
    !is.na(stri_match_all_regex(datcompleto$lga, "Rural")),
    "Rural",
    "Others"
  )
)
table(datcompleto$fe_type_location) # Obtenemos tres categorias

#- Eliminamos los substrings "Urban" y "Rural" de la columna lga
datcompleto[, lga := stri_replace_all_regex(lga, "Rural|Urban", replacement = "")] 
...
# guardo submission
fwrite(my_sub_9, file = "./submissions/09_05_lumping_sobre_funder_ward_fe_urban_rural.csv")

#-- Pintar importancia de las variables
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/09_05_lumping_sobre_funder_ward_fe_urban_rural.png')

knitr::kable(data.frame("Train accuracy" = c(0.8168687, 0.8101178, 0.8122391, 0.8124579, 0.8122727, 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8158418), 
                        "Data Submission" = c(0.8128, 0.8096, 0.8174, 0.8176, 0.8168, 0.8197, 0.8213, 0.8203, 0.8198, 0.8195),
                        row.names = c("Num + Cat (> 1 & < 1000) sin duplicados",
                                      ...
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana) + fe type_location")),
             align = 'c')

#-- Conclusion: el hecho de si el poblado es rural o urbano no aporta mejora al modelo