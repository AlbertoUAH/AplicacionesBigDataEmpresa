...
suppressPackageStartupMessages({
  ...
  library(missForest)           # Imputacion missings
  library(doParallel)           # Procesamiento paralelo
...
#   ¿Y si aplicamos missForest sobre el conjunto train+test?
...
#-- Aplicamos miss ranger sobre las columnas (imputacion multiple)
registerDoParallel(cores = detectCores())
datcompleto_imp <- missForest(
  datcompleto,  
  maxiter = 10,
  ntree = 100, 
...
datcompleto_imp_final <- datcompleto_imp$ximp

# Comprobamos valores missings
sum(is.na(datcompleto_imp_final))

# Comprobamos que el orden del dataset no ha cambiado
which(datcompleto_imp_final$id == 50785) == fila_test

#-- Modelo
# Dividimos entre conjunto de entrenamiento y prueba
train <- datcompleto_imp_final[c(1:fila_test-1),]
train$status_group <- vector_status_group
train$status_group <- as.factor(train$status_group)

test <- datcompleto_imp_final[c(fila_test:nrow(datcompleto_imp_final)),]
...
my_sub_12 <- make_predictions(my_model_12, test)
# guardo submission
fwrite(my_sub_12, file = "./submissions/12_05_lumping_sobre_funder_ward_freq_abs_categoricas_num_imp_miss_forest.csv")
# 0.8211

knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8157071, 0.8156229, 0.8153199), 
                        "Data Submission" = c(0.8180, 0.8197, 0.8213, 0.8203, 0.8198, 0.8216, 0.8226, 0.8216, 0.8227),
...
#-- Conclusion: la imputacion de missings mediante un missForest tampoco ha permitido una "ligera" mejoria en cuanto a accuracy se refiere (0.8227), pero
#   dado el tiempo de computo que ha requerido, no es tecnica que merezca especialmente la pena dada la pequeña mejoria que supone.
...
# Las columnas "_na" son poco relevantes
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/12_05_lumping_sobre_funder_ward_freq_abs_categoricas_num_imp_miss_forest.png')
