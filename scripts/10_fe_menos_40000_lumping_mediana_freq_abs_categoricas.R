...
#-- subvillage
#-- Aplicamos lumping sobre la mediana (50 % de las categorias con una proporcion menor a 7e-04)
summary(c(prop.table(table(datcompleto[, fe_subvillage]))))
datcompleto[, fe_subvillage := fct_lump_prop(datcompleto[,fe_subvillage], 2.7e-05, other_level = "other")]
...
# solo freq. abs.      0.8162963
# variable sin imp.    0.8160606    
my_model_14 <- fit_random_forest(formula, train)

my_sub_14 <- make_predictions(my_model_14, test)
# guardo submission
fwrite(my_sub_14, file = "./submissions/14_13_lumping_sobre_funder_ward_installer_scheme_name_freq_abs_categoricas_mas_subvillage.csv")
# lumping + freq.abs. 0.8211
# solo lumping        0.8219
# solo freq. abs.     0.8235
# variable sin imp.   0.8238
...
knitr::kable(data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8162121, 0.8154882, 0.8157071, 0.808771, 0.7995118,
                                             0.8155556, 0.8168182, 0.8161616, 0.8160606), 
...
# El hecho de incluir la variable subvillage ha provocado que la precision del modelo empeore, de 0.8239 a 0.8211
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/14_13_lumping_sobre_funder_ward_installer_scheme_name_subvillage_freq_abs_categoricas.png')
