...
#         1. Reemplazar las variables fe_funder y fe_ward por el promedio de aparicion con respecto a la variable objetivo 
# Comentarios: 
#-------------------
...
train <- as.data.table(target_encode(train, target_encoding = dat_encoded))
test <-  as.data.table(target_encode(test, target_encoding = dat_encoded))
sum(is.na(test)) # 21 valores missing en status_group_by_fe_funder

# Reemplazamos los valores nulos por la media
media <- mean(test$status_group_sum_by_fe_funder, na.rm = TRUE)
setnafill(test, cols="status_group_sum_by_fe_funder", fill = media)

# Añadimos ruido a los datos train
cat_cols <- paste0('status_group_sum_by_',names(datcat_df))
