...
1. Selección de variables numéricas y categóricas (con al menos dos categorías y menos de 1000), además de descartar las variables duplicadas _payment_type_ y _quantity_group_, además de _recorded_by_ (con una única categoría), obteniendo la siguiente puntuación:
...
Además, durante toda la práctica los modelos __se han realizado con la misma semilla__: 1234.
...
suppressPackageStartupMessages({
  ...
  library(knitr)           # Generacion de documentos + formateo de tablas
...
__En esta sección se comentará las aportaciones que realicé en el grupo durante el concurso para mejorar la puntuación obtenida en clase,__ dado que cada miembro del grupo realizó sus pruebas de forma independiente.
...
numlev_df <- numlev_df %>% arrange(levels)

color_me <- which(numlev_df$levels < 1000 & numlev_df$levels > 1)
kable(numlev_df) %>%
  kable_styling() %>%
...
2. _gps_height_: en el caso de la altura GPS, __no se han considerado los valores negativos como valores anómalos__, principalmente porque, trasladando el problema a un entorno real, es posible la existencia de alturas negativas (regiones situadas por debajo del nivel del mar).
...
color_me <- which(puntuaciones$Data.Submission == 0.8128)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
knitr::include_graphics('../charts/02_num_cat_menos1000_fe_cyear_dist_cant_agua.png')
```
...
color_me <- which(puntuaciones$Data.Submission == 0.8174)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
```{r, echo=FALSE, fig.align='center'}
#- Guardamos el grafico con la importancia del modelo
knitr::include_graphics('../charts/03_num_cat_menos1000_fe_cyear_dist_cant_agua_dr_year_month_old.png')
```
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Train accuracy" = c(0.8168687, 0.8101178, 0.8105724, 0.8122391, 0.8115152), 
...
color_me <- which(puntuaciones$Data.Submission == 0.8176)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```

Observamos que __el modelo ha mejorado de 0.8174 a 0.8176__. Sin embargo, no todas las variables son de especial importancia, como es el caso de _fe_dr_year_ (ver gráfica anterior) ¿Y si eliminamos dicha variable?
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Train accuracy" = c(0.8168687, 0.8101178, 0.8105724, 0.8122391, 0.8124579, 0.812138), 
...
Prácticamente, obtuvimos el mismo resultado eliminando el año de _date_recorded_, por lo que se decidió no tener en cuenta dicha variable, al no aportar mejoría alguna.
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Train accuracy" = c(0.8168687, 0.8101178, 0.8105724, 0.8122391, 0.8124579, 0.812138, 0.8137374), 
...
Pese al tuneo realizado, no consigue mejorar el _score_ obtenido en _Feature Engineering_.
...
3. ___Feature Engineering_: _cyear_, _dist_, _cant_agua_, _dr_month_ y _dr_year_ - _cyear___
4. __Empleando un modelo _Random Forest_ estándar: 500 árboles y mtry = 6__ (raíz cuadrada del número de variables)

Sobre dicho modelo, se ha obtenido una mejor puntuación de 0.812138 en el conjunto _train_ y 0.8176 en el conjunto de prueba, por lo que el objetivo consistirá en superar dicho _accuracy_, así como la mejor puntuación obtenida por el grupo ganador (0.8248).
...
# Inicialmente los cargamos en formato data.frame
dattrainOr       <- fread(file = "../data/train_values.csv", data.table = FALSE)
...
```{r, echo=FALSE}
knitr::include_graphics('../charts/05_04_mas_fe_inst_funder_scheme_ward.png')
```
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212),
                        row.names = c("Mejor accuracy en el concurso",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)"))

color_me <- which(puntuaciones$Data.Submission == 0.8212)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8159259, 0.8146633), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212, 0.8196, 0.8203),
                        row.names = c("Mejor accuracy en el concurso",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)",
...
De este modo, en caso de que alguna de las técnicas anteriores, sobre las variables _ward_ y _funder_, mejore el _score_ (0.8212), __se aplicará al resto de variables categóricas__.
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8159259, 0.8146633, 0.8158418), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212, 0.8196, 0.8203, 0.8214),
                        row.names = c("Mejor accuracy en el concurso",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)",
                                      "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (primer cuartil)",
...
color_me <- which(puntuaciones$Data.Submission == 0.8214)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
Aparentemente, el valor obtenido en el conjunto de entrenamiento mejora relativamente (de 0.815 a 0.816). Sin embargo, al predecir el conjunto _test_, observamos que el _score_ empeora:
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8159259, 0.8158418, 0.8154882), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212, 0.8203, 0.8196, 0.8214, 0.8216),
                        row.names = c("Mejor accuracy en el concurso",
                        "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (tercer cuartil)",
...
color_me <- which(puntuaciones$Data.Submission == 0.8216)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8159259, 0.8158418, 0.8154882, 0.8154377), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212, 0.8203, 0.8196, 0.8214, 0.8216, 0.8164),
                        row.names = c("Mejor accuracy en el concurso",
                        "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (tercer cuartil)",
...
Como podemos comprobar en la tabla resultante, aplicando _Target Encoding_ ha empeorado los resultados.

#### 3.1.6.2 Empleando la suma

Revisando la documentación de la función _build_target_encoding_, me llamó la atención un ejemplo propuesto en el que se emplea la función de agregación _sum_ en lugar de la media. A modo de ejemplo, supongamos el siguiente _data.table_ con diferentes bombas de agua:
...
puntuaciones <- data.frame("Train accuracy" = c('-', 0.8149832, 0.8159764, 0.8146633, 0.8159259, 0.8158418, 0.8154882, 0.8154377, 0.814899), 
                        "Data Submission" = c(0.8176, 0.8197, 0.8212, 0.8203, 0.8196, 0.8214, 0.8216, 0.8164, 0.8215),
                        row.names = c("Mejor accuracy en el concurso",
                        "Num + Cat (> 1 & < 2100) fe anteriores + fe_funder + fe_ward",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (mediana)",
                        "Num + Cat (> 1 & < 2100) fe anteriores + lumping sobre funder + ward (tercer cuartil)",
...
Antes de realizar la imputación, guardamos en una variable el número de valores únicos de cada columna, con el fin de contrastar posteriormente la variación del número de categorías:
...
color_me <- which(puntuaciones$Data.Submission == 0.8226)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
### 3.1.9 Añadiendo las variables scheme_name e installer
...
color_me <- which(puntuaciones$Data.Submission == 0.8239)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
```{r, echo=FALSE}
datcompleto <- fread("../temp_datcompleto_3.csv", data.table = TRUE)
```
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993), 
                        "Data Submission" = c(0.8239, 0.8219),
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8162963), 
                        "Data Submission" = c(0.8239, 0.8219, 0.8220),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 40000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer)",
...
Incluso sin depurar la variable, el _score_ obtenido continua siendo menor al mejor modelo obtenido. No obstante, podemos ir analizando la evolución del _score_ conforme aplicamos técnicas de depuración como _clean_text_ o _lumping_:

```{r, echo=FALSE}
puntuaciones <- data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8162963, 0.81633, 0.816633), 
                        "Data Submission" = c(0.8239, 0.8219, 0.8220, 0.8222, 0.8221),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 40000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer)",
                                      "Sin depurar subvillage",
                                      "Solo clean_text",
                                      "clean_text + lumping"))
...
Pese al haber aplicado funciones como _clean_text_ o _lumping_ (sin imputar por frecuencias aboslutas sobre _subvillage_), no nos proporciona mejoría alguna al modelo, __por lo que descartamos añadir dicha columna__.
...
```{r, echo=FALSE}
datcompleto <- fread("../temp_datcompleto_4.csv", data.table = TRUE)
```
...
__Cabe destacar la considerable disminución en el número de categorías__, por lo que es posible que estemos perdiendo información relevante de cara al modelo, del mismo modo que sucedía con _subvillage_. De hecho, si entrenamos el nuevo modelo:
...
```{r, echo=FALSE}
kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993), 
                        "Data Submission" = c(0.8239, 0.8236),
                        row.names = c("Mejor accuracy hasta el momento",
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8155892), 
                        "Data Submission" = c(0.8239, 0.8236, 0.8220),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 60000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer y subvillage)",
...
Por el contrario, si mantenemos la variable original, el hecho de disponer de un elevado número de categorías provoca una disminución en el _score_ del modelo ¿Y si en lugar de aplicar la imputación por freq. abs. sobre _wpt_name_ aplicamos tan solo _clean_text_ o _clean_text_ más _lumping_?

```{r, echo=FALSE}
puntuaciones <- data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8155892, 0.8153367, 0.8167508), 
                        "Data Submission" = c(0.8239, 0.8236, 0.8220, 0.8207, 0.8227),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 60000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer y subvillage)",
                                      "Sin depurar wpt_name",
                                      "Solo clean_text",
...
Incluso aplicando _clean_text_ y/o _lumping_, el _score_ del modelo no mejora, a la vista de los resultados anteriores, por lo que __descartamos la variable _wpt_name___. Por tanto, de las nuevas variables con un elevado número de categorías __nos quedaremos únicamente con _ward_, _funder_ y _scheme_name___:

```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616), 
                        "Data Submission" = c(0.8239),
...
```{r, echo=FALSE}
datcompleto <- fread("../temp_datcompleto_5.csv", data.table = TRUE)
dattestOr   <- fread("../dattestOr.csv", data.table = TRUE)
```
...
```
```{r, echo=FALSE}
datcompleto_imp <- fread("../datcompleto_imp.csv", data.table = TRUE)
```
```{r}
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855), 
                        "Data Submission" = c(0.8239, 0.8251),
                        row.names = c("Mejor accuracy hasta el momento",
...
No obstante, si nos fijamos en el gráfico de importancia resultante:

```{r, echo=FALSE, fig.align='center', fig.height=12, fig.width=12}
knitr::include_graphics('../charts/19_lumping_fe_freq_abs_sobre_funder_ward_scheme_name_resto_categoricas_y_permit_public_meeting.png')
```
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855, 0.8171886), 
                        "Data Submission" = c(0.8239, 0.8251, 0.8235),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 10000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer) + variables logicas",
...
kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855, 0.8171886, 0.8168519, 0.8172559), 
                        "Data Submission" = c(0.8239, 0.8251, 0.8235, 0.8234, 0.8236),
                        row.names = c("Mejor accuracy hasta el momento",
                                      "Num + Cat (> 1 & < 10000) fe anteriores + lumping (mediana) + freq. abs. cat. (sin installer) + variables logicas",
                                      "Eliminando is_na_public_meeting e is_na_permit",
...
color_me <- which(puntuaciones$Data.Submission == 0.8251)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064), 
                        "Data Submission" = c(0.8251, 0.8236),
...
kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657), 
                        "Data Submission" = c(0.8251, 0.8236, 0.8224),
  row.names = c("Mejor modelo hasta el momento",
  "Añadiendo year (date_recorded)",
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/20_lumping_fe_freq_abs_sobre_funder_ward_scheme_name_resto_categoricas_mas_logicas_fe_otros_campos.png")
```
...
kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212), 
                        "Data Submission" = c(0.8251, 0.8236, 0.8224, 0.8226),
  row.names = c("Mejor modelo hasta el momento",
  "Añadiendo year (date_recorded)",
  "Incluyendo otros campos",
...
kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212, 0.8175084), 
                        "Data Submission" = c(0.8251, 0.8236, 0.8224, 0.8226, 0.8242),
  row.names = c("Mejor modelo hasta el momento",
  "Añadiendo year (date_recorded)",
  "Incluyendo otros campos",
  "Solo dr_qday, dr_day, dr_week y dr_wday",
...
kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212, 0.8175084, 0.8170202), 
                        "Data Submission" = c(0.8251, 0.8236, 0.8224, 0.8226, 0.8242, 0.8243),
  row.names = c("Mejor modelo hasta el momento",
  "Añadiendo year (date_recorded)",
  "Incluyendo otros campos",
  "Solo dr_qday, dr_day, dr_week y dr_wday",
...
puntuaciones <- data.frame("Train accuracy" = c(0.8168855, 0.8168855), 
                        "Data Submission" = c(0.8251, 0.8247),
  row.names = c("Mejor modelo hasta el momento",
  "Añadiendo day_count"))
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/random_forest_seleccion_ntree.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/random_forest_seleccion_ntree400_mtry_tunning.png")
```

__Empleando 500 árboles__:

```{r, echo=FALSE}
knitr::include_graphics("../charts/random_forest_seleccion_ntree500_mtry_tunning.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/random_forest_seleccion_max_depth.png")
```
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Data Submission" = c(0.8251, 0.8253),
  row.names = c("Mejor score Feature Egineering",
  "Mejor score Random Forest"))

color_me <- which(puntuaciones$Data.Submission == 0.8253)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/xgboost_nrounds.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/xgboost_max_depth_ntrees.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/xgboost_eta_parameter.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("../charts/xgboost_nrounds_eta_parameter_tunned.png")
```
...
Por último, y empleando 600 iteraciones, _max_depth_ = 15 y _eta_ = 0.02, procedemos a tunear el último parámetro, _colsample_bytree_, el cual hemos mantenido por defecto a 0.3. De este modo, __comprobamos si sorteando un mayor o menor número de variables mejora el _score_ del modelo__:

```{r, echo=FALSE}
knitr::include_graphics("../charts/xgboost_colsample_by_tree.png")
```
...
Como conclusión, con _XGboost_ empleamos la siguiente configuración:
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Data Submission" = c(0.8251, 0.8253, 0.8260),
  row.names = c("Mejor score Feature Egineering",
  "Mejor score Random Forest",
  "Mejor score XGboost"))

color_me <- which(puntuaciones$Data.Submission == 0.8260)
kable(puntuaciones) %>%
  kable_styling() %>%
  row_spec(color_me, bold = TRUE, background = "#4D934D")
```
...
knitr::include_graphics("../hexagon_logo.svg")
```

No obstante, antes de finalizar con _XGboost_ realizamos una última prueba, concretamente con un paquete denominado _autoxgboost_, desarrollado por _Janek Thomas_. Dicho paquete permite realizar, de un modo similar a _h2o_, el tuneo automático del mejor modelo a través de una optimización bayesiana:
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Data Submission" = c(0.8251, 0.8253, 0.8260, 0.8206),
  row.names = c("Mejor score Feature Egineering",
  "Mejor score Random Forest",
  "Mejor score XGboost",
  "autoxgboost"))
...
```{r, echo=FALSE}
puntuaciones <- data.frame("Data Submission" = c(0.8251, 0.8253, 0.8260, 0.8206, 0.8101, 
                                                 0.8118, 0.8020, 0.8079, 0.8137, 0.8077,
                                                 0.8027, 0.8137, 0.7945, 0.6805),
                row.names = c("Mejor score Feature Egineering",
                "Mejor score Random Forest",
                "Mejor score XGboost",
                "autoxgboost",
...
Finalmente, y en base a los resultados obtenidos a lo largo de la práctica, __el mejor _score_ obtenido ha sido de 0.8260__:
