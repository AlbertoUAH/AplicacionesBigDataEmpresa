...
Nuevamente, el número de valores únicos se ve reducido de 21426 a 21295. A continuación, aplicamos _lumping_ e imputación por frecuencias absolutas:
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8162963), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8162963, 0.81633, 0.816633), 
...
Pese al haber aplicado funciones como _clean_text_ o _lumping_ (sin imputar por frecuencias aboslutas sobre _subvillage_), no nos proporciona mejoría alguna al modelo, __por lo que lo descartamos__.

### 3.1.11 Añadiendo wpt_name

Junto con _subvillage_, nos queda por añadir una variable con un elevado número de categorías: _wpt_name_, con 37.400 valores únicos. Del mismo modo que en el apartado anterior, aplicamos el mismo proceso de depuración sobre dicha variable.
...
Sorprendentemente, pese al haber reducido drásticamente el número de categorías, el _score_ obtenido no se aleja tanto del mejor modelo hasta el momento (0.8236 frente a 0.8239) ¿Y si mantenemos la variable _wpt_name_ original? Es decir, sin depurar:

```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8155892), 
...
Por el contrario, si mantenemos la variable original, el hecho de dispone de un elevado número de categorías provoca una disminución en el _score_ del modelo ¿Y si en lugar de aplicar la imputación por freq. abs. sobre _wpt_name_ aplicamos tan solo _clean_text_ o _clean_text_ más _lumping_?

```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8165993, 0.8155892, 0.8153367, 0.8167508), 
...
Incluso aplicando _clean_text_ y/o _lumping_, el _score_ del modelo no mejora, a la vista de los resultados anteriores, por lo que __descartamos la variable _wpt_name___. Por tanto, de las nuevas variables con un elevado número de categorías nos quedaremos con _ward_, _funder_ y _scheme_name_:
...
### 3.1.12 Añadiendo variables lógicas permit y public_meeting

Como últimas variables, nos quedan por añadir _permit_ y _public meeting_, ambas variables lógicas. Sin embargo, nos encontramos un problema: __ambas columnas presentan valores _missing___.
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855), 
...
Añadiendo ambas variables lógicas, el modelo ha mejorado de 0.8239 a 0.8251, superando incluso a la mejor puntuación obtenida durante el concurso en clase (0.8248).
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855, 0.8171886), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8161616, 0.8168855, 0.8171886, 0.8168519, 0.8172559), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657), 
...
__Nota__: para una mejor visualización, se han eliminado el resto de variables.

Podemos observar que variables como el cuatrimestre o si es o no fin de semana no presentan una importancia demasiado relevante, lo que podría estar provocando una disminución del _score_ final ¿Mejoraría el modelo si descartamos ambas columnas?

```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212, 0.8175084), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8173064, 0.8165657, 0.8171212, 0.8175084, 0.8170202), 
...
```{r, echo=FALSE}
knitr::kable(data.frame("Train accuracy" = c(0.8168855, 0.8168855), 
...
Tras realizar todos los pasos anteriores, por mejor _score_ obtenido nos decantamos por el siguiente:

1. Seleccionar variables numéricas

2. Seleccionar variables categóricas (con menos de 10000 categorías), __a excepción de _installer___.
...
5. Añadir variables lógicas _permit_ y _public_meeting_, imputando NAs por _missRanger_.
