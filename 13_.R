...
# Salida: Datos con nuevas variables (incluyendo categorias < 10000) + lumping + transformacion fe_funder, fe_ward, lga, scheme_name
#         1. Añadir variables logicas punlic_meeting y permit
# Comentarios: 
#-------------------
...
datcompleto_imp_mice <- mice(datcompleto,
                        method = "logreg.boot", # Regresion logistica con reemplazamiento
...
# Empleando ambas variables logicas (+ col is na) + missRanger   : 0.8173906
# Empleando ambas variables logicas (+ col is na) + mice         : 0.8166498
...
# guardo submission
fwrite(my_sub_17, file = "./submissions/17_13_lumping_sobre_funder_ward_scheme_name_freq_abs_categoricas_mas_logicas_permit_public_meeting_mas_is_na.csv")
# Empleando ambas variables logicas (+ col is na) + missRanger   : 0.8245
# Empleando ambas variables logicas (+ col is na) + mice         : 0.8225
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/17_13_lumping_sobre_funder_ward_scheme_name_freq_abs_categoricas_mas_logicas_permit_public_meeting_mas_is_na.png')
